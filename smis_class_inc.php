...
class smis
{
	...
	// 		$tnsName = "(DESCRIPTION = (ADDRESS_LIST = (ADDRESS = (PROTOCOL = TCP)(HOST = proddb.uonbi.ac.ke)(PORT = 1521))) (CONNECT_DATA = (SERVICE_NAME = proddb)))";
	// 	else
	// 		$tnsName = "(DESCRIPTION = (ADDRESS_LIST = (ADDRESS = (PROTOCOL = TCP)(HOST = proddb.uonbi.ac.ke)(PORT = 1521))) (CONNECT_DATA = (SERVICE_NAME = proddb)))";
	...
	// 		$repeatcase = $this->Checkrepeatcases($this->CCGetSession("regNo"), $this->StudentDetails['CURRENT_LEVEL']);
	...
	// 	} else {
	// 		print "<4>Error connecting to the dbatabase </h4>";
	// 	}
	...
	public function smis($AcademicYear)
	{
		...
		if (function_exists('NewADOConnection')) {
			...
			$this->Ora_Db = NewADOConnection('oci8');
			$connected = $this->Ora_Db->Connect($tnsName, $loggedonUser, $logonPassword);

			if (!$connected || !$this->Ora_Db->IsConnected()) {
				//echo "<h3 style='color:red;'>Failed to connect to Oracle DB: " . $this->Ora_Db->ErrorMsg() . "</h3>";
				return;
			} else {
				//echo "<h3 style='color:green;'>Successfully connected to Oracle DB</h3>";
			}
			...
			// Student details and session initialization
			$this->RegNo = $this->CCGetSession("regNo");
			$this->StudentDetails = $this->getOtheStudentdetails($this->RegNo);
			$this->KCSEYear = $this->StudentDetails['KCSE_YEAR'];
			...
			if (is_null($this->KCSEYear)) {
				$last4 = substr($this->RegNo, -4);
				$reg_trip = explode('/', $this->RegNo);

				if ($last4 == 2017) {
					$this->KCSEYear = ($reg_trip[1] > 6000) ? 2016 : 2015;
				} elseif ($last4 == 2018) {
					$this->KCSEYear = 2017;
				} else {
					$this->KCSEYear = 2015;
			...
			$repeatcase = $this->Checkrepeatcases($this->RegNo, $this->StudentDetails['CURRENT_LEVEL']);

			$this->Academicyear = $this->CCGetParam("AcademicYear") ?: $this->Academicyear;

			//http://41.89.92.242:8080/site/form
			//http://localhost:8080
			$url = 'https://41.89.92.242/site/form?' . http_build_query([
				'registration_number' => $this->RegNo,
				'full_name' => ucwords(strtolower($this->CCGetSession("otherNames") . " " . $this->CCGetSession("surname"))),
				'degree_code' => $this->StudentDetails['D_PROG_DEGREE_CODE'],
				'level_of_study' => $this->StudentDetails['CURRENT_LEVEL'],
				'academic_year' => $this->Academicyear,
				'academic_session_id' => $this->CCGetParam('AcademicSessionID'), // $this->SessionID ?: 7722,
				'uon_email' => $this->CCGetSession("uon_email"),
				'mobile_number' => $this->CCGetSession("primary_mobile"),
				'gender' => $this->StudentDetails['SEX'],
				'stc_student_category_id' => $this->StudentDetails['STC_STUDENT_CATEGORY_ID']
			]);
			echo '
			<div>
			...
			// echo "</pre>";
			// echo "<h1>Current Academic Year: $this->Academicyear</h1>";
			...
			if (strlen($this->Academicyear)) {
				$LevelofStudy = $this->getCurrentStudyLevel($this->RegNo, $this->Academicyear);
				$AcademicSessionID = $this->SessionID;

				$this->CCSetSession("LevelofStudy", $LevelofStudy);
				$this->CCSetSession("AcademicSessionID", $AcademicSessionID);

				$this->StudentStudySession = $this->GetStudentSesions(
					$this->StudentDegreeCode,
					$this->Academicyear,
					$LevelofStudy,
					$AcademicSessionID
				);
			} else {
				$lofsArray = explode('/', $this->RegNo);
				$y = $lofsArray[2];
			...
			print "<h4>Error: ADOdb not available.</h4>";
		}
	}
	public function GetCurrentAcademicYear()
	{
		...
		$SQLStmt = "SELECT DISTINCT ACADEMIC_SESSION.SESSION_ID,ACADEMIC_SESSION.ACADEMIC_YEAR,SAS.DEGREE_CODE, 
                ...
                where SAS.SESSION_ID = ACADEMIC_SESSION.SESSION_ID   AND  ACADEMIC_SESSION.KCSE_YEAR = $this->KCSEYear 
    ...
    */
	public function GetRunningAcademicYear()
	{
		...
		$SQLStmt = "SELECT  ACADEMIC_SESSION.SESSION_ID,ACADEMIC_SESSION.ACADEMIC_YEAR,SAS.DEGREE_CODE, 
                    ...
                    and  (SAS.DEGREE_CODE LIKE SUBSTR('$Regno',0,3) OR SAS.DEGREE_CODE LIKE '$DegCode') AND SAS.YEAR_OF_STUDY =$LevelofStudy
                    $this->SessionFilter 
                     AND ((SYSDATE BETWEEN TO_DATE(SAS.APPLICATION_START_DATE) AND  TO_DATE(SAS.APPLICATION_END_DATE)+1 )
                     OR (SYSDATE BETWEEN TO_DATE(SAS.BOOKING_START_DATE) AND  TO_DATE(SAS.BOOKING_END_DATE)+1 )
	...
	public function GetThePreviousAcademicYear()
	{
		...
		$SQLStmt = "SELECT  ACADEMIC_SESSION.SESSION_ID,ACADEMIC_SESSION.ACADEMIC_YEAR,SAS.DEGREE_CODE, 
                    ...
                    from SWA.ACADEMIC_SESSION,  SWA.student_academic_sessions   SAS  
                    where SAS.SESSION_ID = ACADEMIC_SESSION.SESSION_ID
                    and  (SAS.DEGREE_CODE LIKE SUBSTR('$Regno ',0,3) OR SAS.DEGREE_CODE LIKE SUBSTR('$Regno ',0,4) ) AND SAS.YEAR_OF_STUDY =$LevelofStudy
	...
	public function GetSystemConfigurations($Academicyear = '')
	{
		...
		echo $DegreeCode . "- " . $Level_of_study . " - " . $Academicyear . "<br>";
        ...
        */
		...
		// // Attempt to connect
		// $this->MysqlSmisDB->Connect('smisdbclone.uonbi.ac.ke', '', '', 'hamis');

		// // Check connection
		// if ($this->MysqlSmisDB->IsConnected()) {
...
class PDF extends FPDF
{
    ...
    */
	...
	function Footer()
	{
		...
		$this->Cell(0, 10, "This invoice is payable to, Bank: Barclays Bank Of Kenya, Branch : Market Branch, Account Name: UON SW Authority, Account No: 0948245590");
		$this->SetY(-193);
		//$this->SetFont('Arial','I',7);
