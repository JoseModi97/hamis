...
if ($_SERVER["HTTP_HOST"] == 'localhost' || $_SERVER["HTTP_HOST"] == 'smisclone.uonbi.ac.ke') {
...
//die(SMIS_RELATIVE_PATH);
include_once('../includes/smis_header_common.php');
...
include RELATIVE_PATH . '/smis_class_inc.php';
//include RELATIVE_PATH . '/class.php';
...
$AcademicYear = $HamisOnline->CCGetParam("AcademicYear") ? $HamisOnline->CCGetParam("AcademicYear") : $HamisOnline->Academicyear;
$AcademicSessionID    = $HamisOnline->CCGetParam("AcademicSessionID") ? $HamisOnline->CCGetParam("AcademicSessionID") : $HamisOnline->SessionID;
...
if ($LevelofStudy == 0  && $StudentCategory == '001') {
	...
	if (strlen($YearofGradaution))
		...
		$HamisOnline->SetError($HamisOnline->GetError() . "<br>You cannot book a room because your Level of Study for the Academic Year $AcademicYear cannot be determined, for more information contact your Faculty/School or Institute ");
...
if ($_SESSION['session'] == 'next') {
	...
	if ($HamisOnline->checkGraduatedLeveofStudy($RegistrationNo, $LevelofStudy) === TRUE) {
		...
		$DisplayInformation .= '<font size="+1" color="blue">You cannot Apply for Accomodation on this Academic year since you are assumed to have Graduated. <br></font>';
	//unset($_SESSION['session']);
}
...
if ($_SESSION['session'] == 'out') {
	if ($HamisOnline->checkGraduatedLeveofStudy($RegistrationNo, $LevelofStudy) === TRUE) {
		if ($StudentCategory == '001') {
			...
			if ($HamisOnline->RoomApplication == 'CLOSED' || $HamisOnline->StudentApplicationStatus === TRUE) {
				$DisplayInformation .= $HamisOnline->studentBookings($StudentsSessions);
			} elseif ($HamisOnline->StudentApplicationStatus === FALSE && !$StudentsSessions) {
				$DisplayInformation .= '<font size="+1" color="blue">The Room Application information for the academic year ' . $AcademicYear . ' has not been set. ,for more information contact the Student Welfare Authority(SWA)<br></font>';
			}
		} else {
...
<div id="left">
	<div class="left_articles">
		<?php
		//echo $LevelofStudy;
		$accomodationInformationDisplied = FALSE;
		if ($Loggedin) {
			if (strlen($_SESSION['session']) == 0) {
				echo '<fieldset align = "center" style = "margin-left:10px; width: 95%; color:black; font-weight:bold;"><legend>Room Application and Confirmation Links</legend>';
				echo '<table><tr><td>';
				echo '<table align = "center"><ul>';

				if ($StudentCategory == '001' && strlen($AcademicYear) && $HamisOnline->DaysToEnd > 0) {
					if ($LevelofStudy == 1) {
						echo '<tr><td><li><a href=?session=next&AcademicYear=' . $AcademicYear . '> Year ' .  $LevelofStudy . ' Regular Students Room Application <font color = "red">' . $AcademicYear . '</font> Academic Year </a></td></li></tr>';
					} else {
						//print displyApplication( $AcademicYear)    ;
						echo '<tr><td><li><a href=?session=in&AcademicYear=' . $AcademicYear . '>Regular Students Room Application  <font color = "red">' . $AcademicYear . '</font> Academic Year</a></td></li></tr>';
						$accomodationInformationDisplied = TRUE;
					}
				}
				if ($StudentCategory == '001' && $HamisOnline->DaysToEnd <=  0 && $HamisOnline->DaysToStart <= 0 && $HamisOnline->AllocationStatus == 'PREVIOUS' && !$accomodationInformationDisplied) {
					//print displyApplication( $AcademicYear)    ;
					echo '<tr><td><li><a href=?session=in&AcademicYear=' . $AcademicYear . '>Regular Students Room Application <font color = "red">' . $AcademicYear . '</font> Academic Year</a></td></li></tr>';
					$accomodationInformationDisplied = TRUE;
				} elseif ($StudentCategory == '001' && $HamisOnline->DaysToEnd <=  0 && $HamisOnline->DaysToStart <= 0 && !$accomodationInformationDisplied) {
					//print displyApplication( $AcademicYear)	;		 
					echo '<tr><td><li><a href=?session=in&AcademicYear=' . $AcademicYear . '>Regular Students Room Application  <font color = "red">' . $AcademicYear . '</font> Academic Year</a></td></li></tr>';
					$accomodationInformationDisplied = TRUE;
					//echo '<tr><td><li><a href=?session=out>Vacational Room Application and Confirmation for '. $AcademicYear . ' current Academic Year</a></li></td></tr>';
				} elseif ($StudentCategory == '001' && is_array($StudentsSessions && !$accomodationInformationDisplied)) {
					//print displyApplication( $AcademicYear)    ;
					echo '<tr><td><li><a href=?session=in&AcademicYear=' . $AcademicYear . '>Regular Students Room Application  <font color = "red">' . $AcademicYear . '</font> Academic Year</a></td></li></tr>';
					$accomodationInformationDisplied = TRUE;
				} elseif ($StudentCategory != '001') {
					echo '<tr><td><li><a href=?session=modII&AcademicYear=' . $AcademicYear . '>Module II Room Application and Confirmation</a></li></td></tr>';
				}
				echo '</ul></table>';
				print '</td><td>';
				// Display all the Previous  Room Allocation Information
				print $HamisOnline->displypreviousallocations();

				print '</td></tr> ';
				echo '</td></tr></table>';
				echo '</fieldset>';
			} else {
				if (($HamisOnline->DaysToEnd <= 0) && ($HamisOnline->DaysToStart > 0))
					$OnlineApplicationStatus = "The Room Application for   Year $LevelofStudy in $AcademicYear will start in $HamisOnline->DaysToStart days ";
				elseif (($HamisOnline->DaysToEnd == 0 && !$StudentsSessions))
					$OnlineApplicationStatus = "The Room Application for year  $LevelofStudy in $AcademicYear has been closed  ";

				if (!$HamisOnline->AllocationStatus) {
					echo '<table align = "center">';
					echo '<tr><td><a href=?home=yes >
                    <font color ="red">' . $OnlineApplicationStatus . ' </font> <br>
                    <font size="+0.5" color="blue">CLICK TO GO BACK TO MAIN MENU</font></a>
            </td></tr>';
					echo '</table>';
				}
			}
