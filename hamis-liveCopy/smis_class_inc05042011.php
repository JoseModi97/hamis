   ...
   public function   smis($AcademicYear){
	...
	public function getBookingDetails($RegNo,$AcademicYear){
		...
		$bookstatusquery="SELECT SRB.booking_id, SRB.reg_no, SRB.hall_code,HL.hall_name, SRB.room_no, SRB.semester,
						...
						start_date,end_date ,confirm_by_date
	...
	public function GetRoomBookingFooter($StudentBookingStatus,$BookingStatus,$RoomCharge,$BalanceBf,$ConfirmableBookingStatus){
	...
	public function DisplayBookingInformation($BookedRoomInfo,$BookingStatus){
        ...
        if($BookingStatus['DAYS_REMAINING'] >= 0 && $BookingStatus['STATUS'] == 'OPEN' ){
            ...
            $ConfirmationDeadline = "You have $ConfimByDate number of days remaining form today to confirm your room, If you do not confirm this room, you shall be deemed to have rejected the offer and the room will be re-allotted to another student."  ; 
		   ...
		   /*
            ...
            foreach($BookedRoomInfo as $Key => $EachSemesterBookingInfo)  {
			    ...
			    foreach($EachSemesterBookingInfo as $Semeseter => $SemesterBookingInfo){
                    ...
                    if($PreviousStatus != $StudentBookingStatus) {                    
                        if(strlen($PreviousStatus)){
                            $BookedRoonDisplay .= $this->GetRoomBookingFooter($PreviousStatus,$BookingStatus,$RoomCharge,$BalanceBf,$ConfirmableBookingStatus) ;
             ...
             if(($PreviousStatus != $StudentBookingStatus) || $FooterDisplayed === FALSE) {
                    ...
                    if(strlen($PreviousStatus))
                        $BookedRoonDisplay .= $this->GetRoomBookingFooter($StudentBookingStatus,$BookingStatus,$RoomCharge,$BalanceBf,$ConfirmableBookingStatus) ;
             } 	
			 $BookedRoonDisplay .=  '</table>' ;
			 return  $BookedRoonDisplay; 
		}else{
      ...
      $NotificationSQL="SELECT MAX(BOOKING_END_DATE) as CLOSING_DATE
        ...
        if($ClosingDate) {
              ...
              if($Today > $ClosingDateTPS ){
                  //Check Individual Confirmation Extentions
                  $RegistrationNo = $this->CCGetSession("RegNo"); 
                  $IndividualConfirmationDate = $this->MysqlHamisDB->GetOne("SELECT confirm_by_date FROM hamis.student_room_bookings 
                                                                    where reg_no='$RegistrationNo' and academic_year='$AcademicYear' and booking_status in('BOOKED','BOOKED_QUEUE')");
                 if(!empty($IndividualConfirmationDate)){
                     $IndividualConfirmationDateTPS =    $this->ConvertMysqldate($IndividualConfirmationDate) ;    
                     if($IndividualConfirmationDateTPS < $Today ){
                       $BookingStatus['STATUS'] = 'CLOSED' ;
                     }else{
                         $DaysRemaining =   $this->conversecondtoDays($IndividualConfirmationDateTPS - $Today) ;
                        return array("STATUS"=>'OPEN',"STARTING_DATE"=>$BookinStartDate,"DAYS_REMAINING"=>$DaysRemaining) ;
                     }        
                 }
