...
class smis  {
	...
	public function   smis($AcademicYear){
		...
		//define('ADODB_ERROR_LOG_DEST','/var/www/html/sms/errors.log');
		//include('/var/www/html/adodb/adodb-exceptions.inc.php');
		...
		if($_SERVER["HTTP_HOST"] == '10.2.21.87' || $_SERVER["HTTP_HOST"]=='smis.uonbi.ac.ke')  
			...
			$tnsName='(DESCRIPTION = (ADDRESS_LIST = (ADDRESS = (PROTOCOL = TCP)(HOST = 10.2.21.27)(PORT = 1521))) (CONNECT_DATA = (SERVICE_NAME = webdb)))' ;
		...
		if(function_exists(NewADOConnection)) {
		   ...
		   if($_SERVER["HTTP_HOST"] == '10.2.21.87' || $_SERVER["HTTP_HOST"]=='smis.uonbi.ac.ke') 
				...
				$MysqlSmisDsn = "mysql://hamis_user:hamis_09@10.2.21.27/smis";
			//$this->MysqlSmisDB = NewADOConnection($MysqlSmisDsn);
			
		   if($_SERVER["HTTP_HOST"] == '10.2.21.87' || $_SERVER["HTTP_HOST"] == 'smis.uonbi.ac.ke') 	
			 $MysqlHamisDsn = "mysql://hamis_user:hamis_09@localhost/hamis";
		   else
			  $MysqlHamisDsn = "mysql://hamis_user:hamis_09@10.2.21.27/hamis"; 
		   ...
		   // echo "Error " . $Ora_Db->ErrorMsg();
			...
			if($_SERVER["HTTP_HOST"] == '10.2.21.87' || $_SERVER["HTTP_HOST"]=='smis.uonbi.ac.ke') 
				...
				$this->MysqlHamisDB->Connect('10.2.21.27','hamis_user','hamis_09','hamis');
			...
			if($_SERVER["HTTP_HOST"] == '10.2.21.87' || $_SERVER["HTTP_HOST"]=='smis.uonbi.ac.ke')
				...
				$this->MysqlSmisDB->Connect('10.2.21.27','hamis_user','hamis_09','smis');
  ...
  public 	public function  CCServerAccess($Ora_Db){
   ...
   public 	public function  print_r_html($arr, $style = "display: none; margin-left: 10px;"){ 
	...
	public function ConvertOracledate($OracleDate){
		...
		$NumericMonth = ConvertMonthtoNumeric($Month) ;
	...
	public function  authenticate($Username,$Passsword){
		...
		if(strlen($StudentInfo['registration_number'])){
		   ...
		   // fwrite($fp,"Returned Level of Study for $Username LevelofStudy =  $LevelofStudy, Duration = $CourseDuration");
			...
			$this->CCSetSession("ReserveMembership", $OtherStudDetails['$ReserveGroupMember']);   
			return $StudentInfo; 
		}
		else{ 
	...
	function GetReserveGroupMembership($RegistrationNo){
		...
		if($ReserveGroup !== FALSE && stelen($ReserveGroup))  
			return ",$ReserveGroup"  ;
		else
		 return '';
	...
	function PrepareSessionClause($StudentSession){
		...
		public function getBookingDetails($RegNo,$AcademicYear){
			...
			$bookstatusquery="SELECT SRB.booking_id, SRB.reg_no, SRB.hall_code,HL.hall_name, SRB.room_no, SRB.semester,
							...
							WHERE (SRB.reg_no='$RegNo' AND SRB.academic_year='$AcademicYear')	
								  AND(SRB.hall_code=HL.hall_code)
							ORDER BY SRB.booking_status DESC,date_booked asc ,time_booked asc ";
			...
			if(is_array($BookedRoomRS) && count($BookedRoomRS) > 0){
				foreach($BookedRoomRS as $key=>$BookedRoomInfo){	
					$BookedRoomList[$BookedRoomInfo['semester']] = $BookedRoomInfo ;
				}
		...
		public function DisplayBookingInformation($BookedRoomInfo,$BookingStatus){
			...
			if(is_array($BookedRoomInfo)){
				...
				$BookedRoonDisplay .= '<caption><b>Student Room Reservation Details for </b><br>' .
									" $RegistrationNo  $Surname  $OtherNames All Correspondences regarding your booking will be emailed through  $UonEmail " .
									'</caption>' ;
				$RoomCharge = 0;
				...
				foreach($BookedRoomInfo as $Semeseter => $SemesterBookingInfo){
				 ...
				 $BookedRoonDisplay .=  '<tr>
											<td colspan="9">TOTAL Room Charge</td>
											<td align="right">'. number_format($RoomCharge,2) .'</td>
									  </tr>' ;
				
				if((in_array($StudentBookingStatus,$ConfirmableBookingStatus)) && $BookingStatus == 'OPEN'){
							 $BookedRoonDisplay .=  '<tr><td colspan="4" align="center"><a href="'.$_SERVER['PHP_SELF'] . '?ConfirmRoom=YES"> Confirm Booking</a></td>
...
public  function ConsolidateTimeSlot($hall_code,$room_no,$slot_no,$AcademicYear,$startdate, $enddate){
		...
		$PrevTimeSliceSQL = "SELECT start_date, end_date
							...
							WHERE (end_date ='$PrevTimeSliceStartDate' )  AND status = 'AVAILABLE' 
		...
		if(strlen($PrevTimeSliceRS ['start_date']))
		{
			...
			$RemoveTimeSlotSQL[$TimeSlotsCount] = "DELETE FROM hamis.available_rooms
											WHERE start_date  ='{$PrevTimeSliceRS[start_date]}'  
											and end_date = '{$PrevTimeSliceRS[end_date]}' 
											AND hall_code ='$hall_code'
											AND room_no ='$room_no'
											AND slot ='$slot_no'
											AND academic_year ='$AcademicYear'";
		...
		$NextTimeSliceSQL = "SELECT start_date, end_date
							FROM hamis.available_rooms
							WHERE (start_date = '$NextimeSliceStEndDate' )  AND status = 'AVAILABLE'
							AND hall_code ='$hall_code'
							AND room_no ='$room_no'
							AND slot ='$slot_no'
							AND academic_year ='$AcademicYear'";							
		...
		if(strlen($PrevTimeSliceRS ['end_date']))
		{
			...
			$TimeSlotsCount = $TimeSlotsCount +1 ;
			$RemoveTimeSlotSQL[$TimeSlotsCount] = "DELETE FROM hamis.available_rooms
											WHERE start_date  ='{$NextTimeSliceRS[start_date]}'  
											and end_date = '{$NextTimeSliceRS[end_date]}' 
											AND hall_code ='$hall_code'
											AND room_no ='$room_no'
											AND slot ='$slot_no'
											AND academic_year ='$AcademicYear'";			
		...
		$SandwichSliceSQL = "SELECT start_date, end_date, status
					FROM hamis.available_rooms
					WHERE start_date  between ('$PrevTimeSliceStartDate'   and  '$NextimeSliceStEndDate') 
					AND hall_code ='$hall_code'
					AND room_no ='$room_no'
					AND slot ='$slot_no'
					AND academic_year ='$AcademicYear'";		
		...
		foreach($SandwichSliceRS as $SanwichKey => $SandwichSliceInfo)
		{
			if(($SandwichSliceInfo['status'] == 'AVAILABLE') || ($SandwichSliceInfo['status'] == 'WAITING CONFIRMATION') ){
				...
				$RemoveTimeSlotSQL[$TimeSlotsCount] = "DELETE FROM hamis.available_rooms
											WHERE start_date = '{$SandwichSliceInfo[start_date]}'  
											and end_date = '{$SandwichSliceInfo[end_date]}' 
											AND hall_code ='$hall_code'
											AND room_no ='$room_no'
											AND slot ='$slot_no'
											AND academic_year ='$AcademicYear'";
...
public  function confirm_booking($RegNo, $AcademicYear,$ConfirmLevel='ALLOCATE') { //CONFIRM_WAITING
			 $SQL = "SELECT booking_id, reg_no, academic_year,semester,degree_code, level_of_study,start_date,end_date,
							hall_code,room_no,daily_room_charge,date_booked,booking_status,invoice_amout
					 FROM hamis.student_room_bookings 
			 ...
			 if(is_array($bookedrooms)) {
				...
				if($BookingStatus == 'CONFIRM_WAITING'){
				 ...
				 if(($key ==0) && ($BookingStatus ='BOOKED')){
					...
					if(!$InNominalroll[InNominalRoll] || $InNominalroll['Balance'] > 0) {
				 ...
				 $TotalInvoiceAmount =  $TotalInvoiceAmount + $InvoiceAmount ;
				  if  ($BookingStatus == 'BOOKED') {	
						...
						$AllocateSQL = $this->SaveAllocation($RegNo,$Semester,$AcademicYear,$OracleDateAlocated,$HallCode,$RoomNo,$OracleExitDate,$DailyCharge,$Module,FALSE);
						array_push($OracleUpdates,$AllocateSQL) ;
						$SQL = "UPDATE hamis.student_room_bookings SET booking_status = 'CONFIRMED',synchronization_flag=1 WHERE booking_id = $BookingID "  ;
						array_push($MysqlUpdates,$SQL); //$this->MysqlHamisDB->execute($SQL);
				  } else if ($BookingStatus == 'BOOKED_QUEUE') {
						$SQL = "SELECT booking_notification.notification_date, room_waiting.reg_no, room_waiting.academic_year, room_waiting.degree_code, room_waiting.level_of_study
								FROM hamis.booking_notification INNER JOIN hamis.room_waiting ON (booking_notification.queue_id = room_waiting.queue_id)
								...
								AND (DATE_ADD(notification_date, INTERVAL 3 DAY) AS notication_date_expiry ) <= CUURDATE;";
						$result  = $this->MysqlHamisDB->execute($SQL);
						if (strlen($result)) {
								$AllocateSQL = $this->SaveAllocation($RegNo,$Semester,$AcademicYear,$OracleDateAlocated,$HallCode,$RoomNo,$OracleExitDate,$DailyCharge,$Module,FALSE);
								array_push($OracleUpdates,$AllocateSQL) ; 
								$SQL = "UPDATE hamis.student_room_bookings SET booking_status = 'CONFIRMED',synchronization_flag=1 WHERE booking_id = $BookingID"      ; /* Update this Record INhe SWA.STUDENT_ROOM_ALLOCATION */
								array_push($MysqlUpdates,$SQL); //$this->MysqlHamisDB->execute($SQL);
						}
				  }
...
public function SaveAllocation($RegistratioNo,$Semester,$AcademicYear,$DateAllocated,$HallCode,$RoomNo,$ExitDate,$DailyCharge,$Module=1,$ExecuteSQL=TRUE){
			   ...
			   if ($Module == 1)
				 {
					...
					$StudentAllocationSQL = "INSERT INTO SWA.STUDENT_ROOM_ALLOCATIONS (REGISTRATION_NUMBER,ROOM_NO,HALL_CODE,SEMESTER,
										ACADEMIC_YEAR,DATE_ALLOCATED,NATURE_OF_ALLOCATION,CHARGE_PER_DAY,OCCUPANCY,USER_NAME) VALUES('$RegistrationNumber',
										'$RoomNo','$HallCode','$Semester','$AcademicYear',
										to_date('$DateAllocated','DD-MON-YYYY'),'$Nature',$DailyCharge,$Occupancy,user)";
...
public function ProcessContinuingRoomApplication($RegistrationNo,$AcademicYear){
		$ApplicationResponce = '';
		$todayis = date("Y-m-d");  
		$nowtime = date("H:i:s"); 
		$Gender =  $this->CCGetSession("gender");  
...
public function RoomApplicationForm ($RegistrationNo,$mobileno,$Gender,$DegreeCode,$LevelofStudy,$UonEmail, $AcademicYear,$ApplicationStatus) {
	...
	if (!strlen($MobileNo))
		...
		$mobiletextbox = '<input type="text" id="MobileNo" name="MobileNo" value="'.$MobileNo.'" readonly="true">';
	...
	$ApplicationForm .= '<form  action="' .$_SERVER['PHP_SELF']. '" method="post" enctype="multipart/form-data" name="ApplyForRoom"> 
							...
							<tr>
								...
								<td>' . $this->LevelofStudyList("AppLevelofStudy", $AppLevelofStudy) .' '.$this->RoomApplicationError['AppLevelofStudy'].'</td>
							</tr>
							<tr>
...
public function RoomApplicationFormFirstYears($RegistrationNo,$mobileno,$Gender,$DegreeCode,$LevelofStudy,$UonEmail, $AcademicYear,$ApplicationStatus) {
	...
	if (!strlen($UonEmail))
		$emailtextbox = '<input type="text" id="Email" name="Email" / value="'.$Email.'">';
	else
		$emailtextbox = '<input type="hidden" id="PostEmail" name="PostEmail" value="'.$UonEmail.'">' . $UonEmail ;
	if (!strlen($MobileNo))
		$mobiletextbox = '<input type="text" id="PostMobileNo" name="PostMobileNo" value="'.$PostMobileNo.'">';
	else
		$mobiletextbox = '<input type="hidden" id="MobileNo" name="MobileNo" value="'. $MobileNo . '" >' . $MobileNo;
	...
	$ApplicationForm .=  '<form name="ApplyForRoom" method="post" action=" ' . $_SERVER['PHP_SELF'] .'">
						  ...
						  <caption class="FacetFieldCaptionTD">
							...
							<tr>
							  ...
							  <td class="FacetDataTD">'. $this->LevelofStudyList("AppLevelofStudy", $AppLevelofStudy) .'Currently Level:(' . $LevelofStudy . ') '.$this->DisplayFormError($this->RoomApplicationError['AppLevelofStudy']).'</td>
							  <td class="FacetColumnTD"><strong>Room Type Preference <strong><span class="style4"><sup>*</sup></span></strong></strong></td>
							  <td class="FacetDataTD">' .
										$this->GetAvailableRoomTypesForStudent($DegreeCode,$Gender,$LevelofStudy,$RoomTypePreference) .
							  ' ' . $this->DisplayFormError($this->RoomApplicationError['RoomTypePreference']). '</td>
							</tr>
							<tr>
...
public function getRoomApplicationDetails($RegNo,$AcademicYear){
		$vstatus = $this->CCGetParam("vstatus");
		$bookstatusquery="SELECT registration_number, academic_year ,level_of_study, mobile_number ,email, postal_code, postal_address ,
   ...
   function DisplayApplicationInformation($RoomAppplicationRecord,$ApplicationStatus) {
		...
		if(is_array($RoomAppplicationRecord) && count($RoomAppplicationRecord)>0){
			...
			$applicationDispalyInfo = '<table border="1" cellspacing="0" cellpadding="5" align="center">' ;
			$applicationDispalyInfo .= '<caption> 
					<b>Room Application Information information for '. $RoomAppplicationRecord['academic_year'] . ' Academic year </b><br>
					Room Application Closing Date : '. $ApplicationStatus['CLOSING_DATE'] .'
				</caption>' ;
	...
	public function studentBookings($StudentsSessions,$Checking=FALSE){
		...
		// print "<h1>" ;
		 ...
		 if(is_array($BookingStatus) && $BookingStatus['STATUS'] == 'OPEN'){
			 ...
			 if($BookedRoom !== FALSE && ( is_array($BookedRoom) && count($BookedRoom)<1 )) {
				 ...
				 // display the Waiting Information
				   ...
				   $this->RoomBookingStatus = TRUE; 
				   $DisplayInformation .= $this->DisplayBookingInformation($BookedRoom,$BookingStatus['STATUS']); // Display Room Booking Information
		 ...
		 }elseif($BookingStatus == 'NO SET DATE' && $StudentCategory =='001'){
   ...
   function GeneratePdf($RegistrationNo,$DisplayName,$AcademicYear,$BookingStatus){
	...
	public function PrintBookingInformation($BookedRoomInfo){				
		if(is_array($BookedRoomInfo)){
			...
			foreach($BookedRoomInfo as $Semeseter => $SemesterBookingInfo){
				$RoomCharge = $RoomCharge +  $SemesterBookingInfo['invoice_amout'] ;
				$AmountPayable = $RoomCharge +  $Balance ;
